...
- 초반: 물과의 싸움. 폐허에서 씨앗을 얻어서 얻은 물로 농사를 짓자

# Time Advance

- 저장 어디서할지 좀더 생각할 것. 현재 하베스트문을 따른다면 가장 잠잘때 모든 맵 업데이트 + 저장임.
...
- 세력: 인간, 뮤턴트, 로봇? -> 비욘드 어스 조화 순수 우월 -> 컬러 생각하기. 순수(인간의 피 와인색) 조화(진균류 소다색) 우월(파이락사이트 노란색)
- 프롤로그 컷신:
- 주인공은 인간 임무 중 상처를 입고 혼자 어디에 도달함
- 농장에 도달한 뒤 ai 인지 어떤 생명체를 만나서 지령을 받아 초반 퀘 진행하게 됨. 티비의 형태? 스타링크 디쉬
- 황무지라고 해야하나? 부흥운동을 하는 걸로 하자.
- 나중에 루트를 바꿀 수 있음.

# Color scheme

- 인간: 새마을 운동 깃발 초록색(황무지 위 피어나는 꽃과 나무)
- 뮤턴트: 멕시코 국장 참고 하지만 초록색이 겹치니 다른거 생각
- 로봇: 빨강. 모스크바 깃발? 로봇이 인간을 창으로 찌르는 모습이면 어때?
- 타일들: 기본 마른 흙 위주.(사막 느낌), 흙에 돋아난 풀(갈색에 가까운 초록), 풀 덮인 타일(초록 갈색에 가까운 초록)
- 콘크리트: 회색
